...
export default function FavoriteCard({ favorite }: FavoriteCardProps) {
  ...
  if (!weather) {
    return (
      ...
      >
        <div className="flex items-center justify-center min-h-[160px]">
          <div className="text-sm text-gray-500 dark:text-gray-400">날씨 정보를 불러올 수 없습니다.</div>
        </div>
      </div>
    )
  }
  ...
  return (
    <div
      onClick={handleClick}
      className="relative w-full cursor-pointer overflow-hidden rounded-3xl border border-gray-100 bg-white p-6 shadow-xl transition-shadow hover:shadow-2xl dark:border-gray-700 dark:bg-gray-800"
      role="button"
      tabIndex={0}
      onKeyDown={(e) => {
        if (e.key === 'Enter' || e.key === ' ') handleClick()
      }}
      aria-label={`${favorite.name} 날씨 상세로 이동`}
    >
