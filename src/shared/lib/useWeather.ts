...
export function useWeather(): UseWeatherReturn {
  ...
  const getWeather = useCallback(async (latitude: number, longitude: number) => {
    ...
    try {
      ...
      if (!apiKey) {
        throw new Error('기상청 API 키가 설정되지 않았습니다. 환경변수에 NEXT_PUBLIC_WEATHER_API_KEY를 설정해주세요.');
      }
      ...
      if (currentHeader.resultCode !== '00') {
        throw new Error(`초단기실황 API 오류: ${currentHeader.resultMsg} (코드: ${currentHeader.resultCode})`);
      }
      if (forecastHeader.resultCode !== '00') {
        throw new Error(`초단기예보 API 오류: ${forecastHeader.resultMsg} (코드: ${forecastHeader.resultCode})`);
      }
      if (dailyHeader.resultCode !== '00') {
        throw new Error(`단기예보 API 오류: ${dailyHeader.resultMsg} (코드: ${dailyHeader.resultCode})`);
      }
      ...
      const temperatureValue = temperatureItem?.obsrValue || temperatureItem?.fcstValue || undefined;
      ...
      // 기상청 API에서 fcstDate는 예보 대상 날짜가 아니라 발표 날짜일 수 있음
      // 따라서 category가 TMN/TMX인 모든 항목을 찾고, 가장 최근 발표의 오늘 날짜 예보를 찾아야 함
      const today = dayjs().format('YYYYMMDD');
      
      // 최저기온 (TMN) 찾기 - 오늘 날짜의 예보 중 가장 최근 것
      const minTempItems = dailyItems.filter(
        (item) => item.category === 'TMN'
      );
      // fcstDate가 오늘 날짜와 같거나 가까운 것 찾기
      const minTempItem = minTempItems.find(
        (item) => item.fcstDate === today
      ) || minTempItems[0]; // 없으면 첫 번째 것 사용
      
      const minTemp = minTempItem 
        ? safeParseFloat(minTempItem.fcstValue, temperature) 
        : temperature;

      // 최고기온 (TMX) 찾기 - 오늘 날짜의 예보 중 가장 최근 것
      const maxTempItems = dailyItems.filter(
        (item) => item.category === 'TMX'
      );
      // fcstDate가 오늘 날짜와 같거나 가까운 것 찾기
      const maxTempItem = maxTempItems.find(
        (item) => item.fcstDate === today
      ) || maxTempItems[0]; // 없으면 첫 번째 것 사용
      
      const maxTemp = maxTempItem 
        ? safeParseFloat(maxTempItem.fcstValue, temperature) 
        : temperature;

      // 시간대별 날씨 데이터 생성 (초단기예보에서 앞으로 6시간)
      const hourlyWeather: HourlyWeather[] = [];
      const uniqueForecastTimes = [...new Set(forecastItems.map(item => item.fcstTime).filter((time): time is string => typeof time === 'string' && time.length > 0))].sort();
      
      // 현재 시간 이후의 시간대만 선택 (최대 6개)
      const currentTimeNum = parseInt(currentTime, 10);
      const futureTimes = uniqueForecastTimes
        .filter(time => {
          const timeNum = parseInt(time, 10);
          return !Number.isNaN(timeNum) && timeNum >= currentTimeNum;
        })
        .slice(0, 6); // 최대 6시간
